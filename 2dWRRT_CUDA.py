...
class RRT:
    ...
    def find_closest_node(self, point):
        ...
        if len(nodes) == 0:
            return None
        # Use CuPy for large trees
        if len(nodes) > 500:  # Tune this threshold as needed
            nodes_cp = cp.asarray(nodes)
            point_cp = cp.asarray(point)
            dists = cp.linalg.norm(nodes_cp - point_cp, axis=1)
            idx = int(cp.argmin(dists).get())
...
if __name__ == "__main__":
    ...
    for scenario_name, scenario in scenarios.items():
        ...
        # Add tqdm here to wrap the range of runs
